...
# Business Problem
SyriaTel is experiencing unprecedented customer attrition, where a significant portion of its subscribers exit its network over time. This exists has a cascading effect on revenue loss, increased marketing cost and, acquisition costs, and eventually a lack of value for money for the customers. The exits are silent, and reflect overtime, and most telecommunication comes to notice when it’s too late. This data analysis project seeks to solve that problem by developing a data-driven solution that predicts exits before they happen and allows SyriaTel to develop appropriate intervention measures. 
...
- Evaluate the models' performance using appropriate metrics on their ability to predict. 
- Identify key drivers of exits and insights fueling customer exits 
- Provide actionable insights that SyriaTel can implement. 
...
1. Our model correctly predicted that 533 loyal customers would stay; it managed to correctly predict that 71 customers would exit and hence caught them.
2. Our Model wrongly caught 37 loyal customers; these aren't exiting, while our model didn't catch 26 customers. 
...
# Final insights.
1. There "Frustration threshold": There is a tipping point where the number of calls correlates with exit, suggesting that if not resolved, it is a primary driver for customer exits. Customer loyalty drops off after the 3rd support call. The exit rate jumps from ~10% at three calls to 45.78% at four calls, eventually exceeding 60% for customers making five or more calls.
